...
std::optional<ChainType> GetNetworkForMagic(const MessageStartChars& message)
{
    ...
    const auto pooltest_msg = CChainParams::PoolTest()->MessageStart();
    ...
    } else if (std::ranges::equal(message, pooltest_msg)) {
        return ChainType::POOLTEST;
    }
    return std::nullopt;
}

/**
 * PoolTest: сеть для тестирования майнинг-пула.
 */
...
public:
    CPoolTestParams()
    {
        m_chain_type = ChainType::POOLTEST;
        consensus.signet_blocks = false;
        consensus.signet_challenge.clear();
        consensus.nSubsidyHalvingInterval = 210000;
        consensus.BIP34Height = 1;
        ...
        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1,111); // Можно изменить префиксы адресов
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1,196);
        base58Prefixes[SECRET_KEY] =     std::vector<unsigned char>(1,239);
        base58Prefixes[EXT_PUBLIC_KEY] = {0x04, 0x35, 0x87, 0xCF};
        base58Prefixes[EXT_SECRET_KEY] = {0x04, 0x35, 0x83, 0x94};

        bech32_hrp = "pbcrt"; // Новый bech32 hrp
    }
};

std::unique_ptr<const CChainParams> CChainParams::PoolTest()
{
    return std::make_unique<const CPoolTestParams>();
}